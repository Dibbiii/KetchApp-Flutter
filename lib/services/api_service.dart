...
class ApiService {
    ...
    switch (response.statusCode) {
      ...
      case 400:
        throw BadRequestException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'Richiesta non valida'
              : 'Richiesta non valida',
        );
      case 401:
        throw UnauthorizedException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'Non autorizzato'
              : 'Non autorizzato',
        );
      case 403:
        throw ForbiddenException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'Accesso negato'
              : 'Accesso negato',
        );
      case 404:
        throw NotFoundException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'Risorsa non trovata'
              : 'Risorsa non trovata',
        );
      case 409:
        throw UserAlreadyExistsException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'L\'utente esiste già.'
              : 'L\'utente esiste già.',
        );
      case 500:
        throw InternalServerErrorException(
          decodedJson is Map
              ? decodedJson['message'] ?? 'Errore interno del server'
  ...
  Future<dynamic> findEmailByUsername(String username) async {
    final response = await http.get(
      Uri.parse('$_baseUrl/users/email/$username'),
      headers: _getHeaders(),
    );
    return _processResponse(response);
  }
          ...
          if (subject['tomatoes'] != null) {
            for (final tomato in subject['tomatoes']) {
              ...
              if (startAt != null && endAt != null) {
                ...
                final notificationStart = start.subtract(
  ...
  Future<List<Tomato>> getTodaysTomatoes() async {
    final today = DateTime.now().toUtc();
    final todayStr =
        "${today.year.toString().padLeft(4, '0')}-${today.month.toString().padLeft(2, '0')}-${today.day.toString().padLeft(2, '0')}";
    final response = await fetchData('users/@me/tomatoes?date=$todayStr');
    final List<dynamic> tomatoesJson = response as List<dynamic>;
    List<Tomato> tomatoes =
        tomatoesJson.map((json) => Tomato.fromJson(json)).toList();
  ...
  Future<List<Achievement>> getUserAchievements() async {
    final response = await http.get(
      Uri.parse('$_baseUrl/users/@me/achievements'),
      headers: _getHeaders(),
    );
  ...
  Future<dynamic> getUserStatistics({
    ...
    final response = await fetchData(
      'users/@me/statistics?startDate=$startDateStr&endDate=$endDateStr',
    );
    return response;
  }
